...
enum StackmatTimerState {
  ...
  ST_LeftHandOnTimer =
      'L', // Left hand is on timer (Not supported by all timers!)
  ST_RightHandOnTimer =
      'R', // Right hand is on timer (Not supported by all timers!)
  ST_BothHandsOnTimer = 'C', // Both hand is on timer, preparing to start (Not
...
public:
  ...
  // Get interpolated time from timer (in ms). This takes the last reported time
  // from timer and adds the milliseconds since it was reported. Will probably
  // not be as accurate as GetTime() but is very good for printing current time
  // to a display that refreshes faster than data is sent from timer, making for
  // a much smoother display. NOTE: If timer is not currently running, then this
  // will simply return the time as last reported by the timer.
  ...
  // Get minutes on the display (For instance, 1:02:555 will return 1), but
  // interpolated (see GetInterpolatedTime() for more)
  uint8_t GetInterpolatedDisplayMinutes();
  // Get seconds on the display (For instance, 1:02.555 will return 2), but
  // interpolated (see GetInterpolatedTime() for more)
  uint8_t GetInterpolatedDisplaySeconds();
  // Get milliseconds on the display (For instance, 1:02.555 will return 555),
  // but interpolated (see GetInterpolatedTime() for more)
